...
def audio_time_magnitude(audio_file):
    ...
    bass_bins = librosa.core.fft_frequencies(sr=sr, n_fft=D.shape[0]).searchsorted(
        bass_range
    )
    mids_bins = librosa.core.fft_frequencies(sr=sr, n_fft=D.shape[0]).searchsorted(
        mids_range
    )
    highs_bins = librosa.core.fft_frequencies(sr=sr, n_fft=D.shape[0]).searchsorted(
        highs_range
    )
...
def populate_onset_stack(audio_path):
    ...
    onset_frames = librosa.onset.onset_detect(
        y=waveform, sr=sample_rate, hop_length=512, backtrack=True
    )
...
def mainMenu():
    ...
    while True:
        ...
        song_text = pygame.font.Font(None, 30).render(
            "Song Select", True, (255, 255, 255)
        )
        ...
        if start_button.collidepoint(mouse_pos) and mouse_click[0] == 1:
            if music_path == "":
                ...
                selectSongReminderText = pygame.font.Font(None, 50).render(
                    "Please select a song.", True, (255, 255, 255)
                )
...
def gameLoop(beats, onsets):
    ...
    class Player(pygame.sprite.Sprite):
        ...
        def draw(self):
            pygame.draw.circle(
                ...
                center=(self.playerLocX, self.playerLocY),
                radius=self.playerRadius,
            )

        def attack(self, horizontalSpeed):
            new_player_projectile = Projectile(
                self.playerLocX,
                self.playerLocY,
                horizontalSpeed,
    ...
    class Enemy(pygame.sprite.Sprite):
        def __init__(self, screen, enemyRadius, enemyColor):
            ...
            self.position = pygame.Vector2(
                random.randint(100, SCREENWIDTH - 100),
                random.randint(100, SCREENHEIGHT * 0.35),
            )
        ...
        def attackBass(self, value):
            ...
            generateCircularProjectiles(
                ...
                color=(
                    0,
                    output_color * PROJECTILEBRIGHTNESS,
                    output_color * PROJECTILEBRIGHTNESS,
                ),
            )
    ...
    def generateCircularProjectiles(
        projectiles, projectile_count, projectile_radius, sourceX, sourceY, angle, color
    ):
    ...
    def hitDetected(
        ...
        distance = math.sqrt(
            (targetX - projectileX) ** 2 + (targetY - projectileY) ** 2
        )
    ...
    playerProjectiles = []
    player1 = Player(
        screen, 10, PLAYERCOLOR, (SCREENWIDTH / 2), (SCREENHEIGHT * 0.9), 0
    )
    ...
    bass, mids, highs = equalizeArrays(
        smoothData(bassChoppy, smoothingFactor),
        smoothData(midsChoppy, smoothingFactor),
        smoothData(highsChoppy, smoothingFactor),
    )
    ...
    while running:
        ...
        if (
            bassIndex >= bassLength
            or midsIndex >= midsLength
            or highsIndex >= highsLength
        ):
            win_condition = True
            game_over = True
        ...
        if not game_over:
            ...
            plane_verts = create_vertex_groups(
                PLANE_SIZE, SUBDIVISIONS, bassValue, midsValue, highsValue
            )
            ...
            if enemy_current_time - start_time >= ENEMYUPDATEDELAY:
                enemy.target = pygame.Vector2(
                    random.randint(50, SCREENWIDTH - 50),
                    random.randint(100, SCREENHEIGHT * 0.3),
                )
            ...
            while (
                bassIndex < len(bass)
                and (bass[bassIndex][0] * 1000) <= attackCurrentTime - TIMINGOFFSET
            ):
                bassIndex += 1
            ...
            while (
                midsIndex < len(mids)
                and (mids[midsIndex][0] * 1000) <= attackCurrentTime - TIMINGOFFSET
            ):
                midsIndex += 1
            ...
            while (
                highsIndex < len(highs)
                and (highs[highsIndex][0] * 1000) <= attackCurrentTime - TIMINGOFFSET
            ):
                highsIndex += 1
            ...
            if highsIndex - search_range >= 0 and highsValue > HIGHSVOLUMETHRESHOLD:
                ...
                end_index = min(
                    highsIndex - 1, len(highs) - 1
                )  # Ensure end_index is within array bounds
                if (
                    highs[highsIndex][1]
                    > max(highs[start_index:end_index], key=lambda x: x[1])[1]
                ):
                    if (
                        highsIndex + 1 < len(highs)
                        and highs[highsIndex][1]
                        >= max(
                            highs[highsIndex + 1 : highsIndex + search_range],
                            key=lambda x: x[1],
                        )[1]
                    ):
            ...
            if midsIndex - search_range >= 0 and midsValue > MIDSVOLUMETHRESHOLD:
                ...
                end_index = min(
                    midsIndex - 1, len(mids) - 1
                )  # Ensure end_index is within array bounds
                if (
                    mids[midsIndex][1]
                    > max(mids[start_index:end_index], key=lambda x: x[1])[1]
                ):
                    if (
                        midsIndex + 1 < len(mids)
                        and mids[midsIndex][1]
                        >= max(
                            mids[midsIndex + 1 : midsIndex + search_range],
                            key=lambda x: x[1],
                        )[1]
                    ):
            ...
            if bassIndex - search_range >= 0 and bassValue > BASSVOLUMETHRESHOLD:
                ...
                end_index = min(
                    bassIndex - 1, len(bass) - 1
                )  # Ensure end_index is within array bounds
                if (
                    bass[bassIndex][1]
                    > max(bass[start_index:end_index], key=lambda x: x[1])[1]
                ):
                    if (
                        bassIndex + 1 < len(bass)
                        and bass[bassIndex][1]
                        >= max(
                            bass[bassIndex + 1 : bassIndex + search_range],
                            key=lambda x: x[1],
                        )[1]
                    ):
            ...
            print(
                f"Bass: {bassValue:.2f}    Mids: {midsValue:.2f}    Highs: {highsValue:.2f}",
                end="\r",
            )
            ...
            current_score_text = current_score_font.render(
                f"{SCORE}", True, (255, 255, 255), (0, 0, 0)
            )
            screen.blit(
                current_score_text,
                (
                    SCREENWIDTH - current_score_text.get_width(),
                    SCREENHEIGHT - current_score_text.get_height(),
                ),
            )
        ...
        # game over condition
            ...
            if win_condition:
                screen.blit(
                    win_text,
                    (
                        SCREENWIDTH // 2 - win_text.get_width() // 2,
                        SCREENHEIGHT // 2 - win_text.get_height() // 2,
                    ),
                )
            else:
                screen.blit(
                    game_over_text,
                    (
                        SCREENWIDTH // 2 - game_over_text.get_width() // 2,
                        SCREENHEIGHT // 2 - game_over_text.get_height() // 2,
                    ),
                )
            ...
            screen.blit(
                final_score_text,
                (
                    SCREENWIDTH // 2 - final_score_text.get_width() // 2,
                    SCREENHEIGHT // 2 - final_score_text.get_height() // 2 + 100,
                ),
            )
            current_time = pygame.time.get_ticks()
